    ...
    <div className="container mx-auto px-6 py-12">
      ...
      <AnimatedSection className="mb-16">
        <Link href={`/blog/${featuredPost.id}`} className="group block">
          <GlassCard className="grid md:grid-cols-2 gap-0 p-0 overflow-hidden border border-white/10 hover:border-blue-500/50 transition-colors duration-500">
            <div 
              className="h-[300px] md:h-auto relative flex items-center justify-center border-r border-white/10 bg-black transition-transform duration-700 group-hover:scale-105 overflow-hidden"
            >
               <Image 
        ...
        {regularPosts.map((post, idx) => (
          <AnimatedSection key={post.id} delay={idx * 0.1} className="h-full">
            <Link href={`/blog/${post.id}`} className="group block h-full">
              <GlassCard className="h-full flex flex-col p-0 overflow-hidden border-white/10 hover:border-blue-500/30 transition-all duration-300 hover:-translate-y-2">
                <div 
