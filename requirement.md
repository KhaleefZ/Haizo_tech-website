...
The system ensures that data (such as portfolio works, blog posts, and client inquiries) is managed centrally and synced in real-time across the applications.
...
2. **Admin Dashboard (`admin-frontend`)**: A secure, "Glass-Functional" internal operations dashboard used by the HaizoTech team to manage projects, tasks, content, and clients.
...
- **Portfolio (Works)**: Dynamically fetches published projects from the backend (`GET /api/works`) to showcase capabilities.
...
- **Content Management (Blogs & Works)**: Data tables allowing Admins to Create, Read, Update, and Delete (CRUD) portfolio items and blog posts. Changes reflect instantly on the public website.
...
- **`Work`**: Portfolio items displayed on the public site.
